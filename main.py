...
def send_message_callback(call):
    ...
    bot.send_message(call.message.chat.id, "History deleted successfully")
...
def stream_command(message):
    ...
    try:
        json_obj = generate_text(user_prompt, max_tokens=256, stream=False, chat_id=message.chat.id, history=True,
